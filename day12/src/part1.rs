...
impl Map {
    ...
    fn hill_climb(&self) -> u64 {
        ...
        let mut candidates = Vec::from([ShortestPath::new(self.start, self.start, 0)]);
        ...
        shortest.get(&self.end).map(|p| p.steps as u64).unwrap_or(0)
    }
...
fn solve(input: Vec<String>) -> u64 {
    Map::from_input(input).hill_climb()
}
